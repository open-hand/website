...
在`应用` 界面中找到`choerodon-front-demo`。选择分支管理，点击`创建分支`，选择`创建feature分支`，填写`issue`号，如`feature-1`。点击`创建`，即可创建一个分支。
...
在`应用` 找到`choerodon-front-demo` 的`仓库地址`。通过git命令拉取生成的项目代码。然后切换到对应分支进行本地开发。
...
在`持续集成` 页面，找到`choerodon-front-demo`。点击阶段跳转到`Gitlab` 查看`ci`执行情况。

**10. 结束分支**　

当`ci`执行通过以后，可以关闭分支，将`feature`分支合并到`develop`分支上。

在`应用` 页面，找到`choerodon-front-demo`。选择分支管理，找到`feature-1` 分支，点击`结束分支`。结束分支后，会触发`develop` 分支的`ci流水线`。

当`develop` 分支的`ci流水线` 通过以后。在`应用版本` 可以找到，`choerodon-front-demo`生成的对应版本。接下来就可以部署了。
...
**2.** 进入`持续交付`模块，选择`应用部署` 进入应用部署界面。
 ...
 -  选择应用：`choerodon-front-demo`。
 -  选择版本：`choerodon-front-demo` 最新的版本。
...
配置信息中包含`image 镜像名`，`preJob 任务`，`env环境变量`，`metrics 监控`，`logs 日志`，`resources 资源限制`。部署时需要修改`preJob` 中关于数据库信息的配置为实际的值以及`env` 中对应的环境变量。
