...
版本是指向客户发布的软件版本，一个版本可能包括多个史诗，以及很多个问题。根据项目周期，可以对软件项目追踪不同的版本，同时可以将对应的问题分配到版本中。最后以版本的维度向客户发布。更多相关信息请参考[发布版本](../../../user-guide/cooperation/work-lists/)。
...
**第一步：** 点击`待办事项`菜单，进入到待办事项页面，在页面左侧展开版本 tab 页，可快速进入[创建史诗](../../../user-guide/cooperation/work-lists/epic)。（路径：待办事项 > 史诗 > 创建史诗）
...
**第一步：** 进入到`模块管理`菜单内，在页面上方点击[创建模块](../../../user-guide/settings/questions/)，输入`模块名称`（必输）、`负责人`（必输）、`模块描述`（可选）、`默认经办人`（必输）等字段即可创建一个新模块。（路径：模块管理 > 创建模块）
...
设置为项目管理提供项目设置、问题链接、快速搜索等相关功能。更多相关信息请参考[设置](../../../user-guide/settings/questions)。

### 项目设置

项目设置主要设置问题编码的项目前缀、以及项目的默认优先级、默认经办人策略（路径：设置 > 项目设置）。更多相关信息请参考[项目设置](../../../user-guide/settings/questions/)。
