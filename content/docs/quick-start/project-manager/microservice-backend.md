...
**第三步：** 点击`选择应用`，找到已经提交的`choerodon-todo-service`。点击`选择版本`，选择`choerodon-todo-service` 最新的版本，点击下一步。
...
**第五步：** 等待实例重启结束，通过`xxxx/eureka/apps` 查看是否成功注册。xxxx为部署register-server时建的域名，详细见[一键部署](../../../installation-configuration/steps/choerodon/)或者[分布部署](../../../installation-configuration/steps/parts/)
