...
组织层是与项目有关的最高层次，对一个组织后的操作，影响该组织与属于该组织的所有项目，不影响其他组织。
...
- **内容：**管理平台用户
- **功能：**可创建、删除、查看详情、编辑、搜索、启用/禁用、解锁、管理是否为LDAP用户
...
- **内容：**将用户分组统一进行管理
- **功能：**可创建、删除、编辑、搜索用户组，可在用户组内添加、删除用户
...
- **内容：**与服务器相对应，为客户提供本地服务的程序，即第三方应用程序
- **功能：**可创建、删除、编辑、详情查看、搜索客户端
...
- **内容：**规定密码强度规则，即密码必须符合的规则；规定密码使用规则
- **功能：**可更新密码策略
...
<h2 id="7">7.角色分配(组织层)</h2>

- **内容：**管理成员（类型：用户/服务）角色
- **功能：**可对成员分配、删除、更改、查看（按成员/角色）角色
- **备注：**所有系统用户初始都被分配了默认的【运营组织】的【组织查看者】的角色
