...
- 升级XWiki
    ...
    1. 下载`Choerodon-wiki`从`0.8.0`升级到`0.9.0`的升级包`Choerodon-wiki-0.9.0-update.rar` ，下载地址为[地址]()。
    ...
    1. 重启XWiki，重启完成之后使用admin用户登录重启wiki系统。
    1. 进入系统设置： 设置--Content--导入 
    ...
    1. 完成上述步骤之后，点击左上角的logo回到系统首页即可完成更新。
