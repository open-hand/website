...
`XWiki-0.13.0版本需要依赖Choerodon微服务框架的0.13.0版本功能，在升级XWiki之前，请确保Choerodon微服务框架已经升级到0.13.0版本。`
...
2. 等待xwiki安装启动完成，使用admin用户登录wiki系统(直接访问wiki系统的域名)，然后点击顶部导航栏右侧的齿轮（设置）按钮，进入系统设置： 设置->内容->导入 ，进入到导入的设置页，然后上传`Choerodon-wiki-0.13.0-update.xar`更新包 ，下载[地址](https://file.choerodon.com.cn/choerodon-install/wiki/choerodon-wiki-0.13.0-update.xar)。

3. 上传成功之后，点击刚上传的xar包，在页面右侧会出现需要导入的包的内容，滚动页面到最下方，点击`导入`按钮，等待导入完成即可。

4. 升级包导入完成后，用户访问会显示404，退出重新登录即可。
