...
- 由于要修数据，停掉前端、后端访问入口，防止修数据过程中访问接口导致的脏数据

- 定时修数据任务job，考虑多pod的修数据情况，建议最好只启动一个pod修数据
...
- **注意:** 请务必跟踪  agile service pod 的日志，直至出现以下日志
...
- **注意:** 请务必跟踪  test manager service pod 的日志，直至出现以下日志
...
- 升级devops-service:
    ...
    helm upgrade devops-service c7n/devops-service \
        --reuse-values \
        --set env.open.AGENT_VERSION="0.24.0" \
        --set SERVICES_HELM_USERNAME=choerodon \
        --set SERVICES_HELM_PASSWORD=password \
        --version 0.24.1 \
        --namespace c7n-system
    ```
