...
- 验证升级
    - 验证命令
        ...
        ```
        curl -s $(kubectl get po -n choerodon-devops-prod -l choerodon.io/release=asgard-service -o jsonpath="{.items[0].status.podIP}"):8031/health | jq -r .status
        ```
    - 出现以下类似信息即为成功升级
        ```
        UP
...
<blockquote class="warning">
0.10版本notify-service使用redis的数据库是：1，0.11版本改为数据库：2。
</blockquote>
...
![](/img/docs/installation-configuration/parts/agile_fix1.png)

- 点击`/v1/fix_data/move_status`右侧按钮进入API测试页面，执行第一个接口

![](/img/docs/installation-configuration/parts/agile_fix2.png)
...
![](/img/docs/installation-configuration/parts/agile_fix3.png)
