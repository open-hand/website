...
- 部署服务
    ...
    helm install c7n/agile-service \
        ...
        --name=agile-service \
        --version=0.5.2 --namespace=choerodon-devops-prod
    ```
    参数名 | 含义 
    --- |  --- 
...
- 部署服务
    ...
    helm install c7n/choerodon-front-agile \
        ...
        --set ingress.host="agile.choerodon.exmple.choerodon.io" \
        --set service.enable=true \
        --set ingress.enable=true \
        --name=choerodon-front-agile \
        --version=0.5.3 --namespace=choerodon-devops-prod
    ```
    参数名 | 含义 
    --- |  --- 
    preJob.preConfig.mysql{}|初始化配置所需manager_service数据库信息
...
- 在访问搭建好的Choerodon的api，`api.exmple.choerodon.io/manager/swagger-ui.html`，选择`iam_service` -> `client-controller` -> `创建client`
  ...
  - 提交以下数据，注意正式搭建时请替换以下值为真实值
        ...
        ```json
        {
            ...
            "scope": "default",
            "secret": "secret",
            "webServerRedirectUri": "http://agile.choerodon.exmple.choerodon.io"
        }
        ```