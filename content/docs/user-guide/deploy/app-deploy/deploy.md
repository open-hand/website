...
在部署主界面，点击工具栏的`批量部署`，右侧弹出批量动部署界面。以下为批量部署应用服务生成实例的步骤：

![image](/docs/user-guide/deploy/app-deploy/images/multi-deployment01.png)
...
### 3.2 资源配置
若要为此次部署生成的实例配置网络和域名，以便能通过外部网络访问到该实例，只需在同一页面中点击 资源配置展开详情页完成配置即可。以下为网络和域名的配置步骤：

![image](/docs/user-guide/deploy/app-deploy/images/multi-deployment02.png)

#### 3.2.1 网络配置

1. 输入网络名称；
2. 选择 `网络类型`，其中包括了 `ClusterIP`、`NodePort`和`LoadBalancer`；
   - ClusterIP:
...
3. 完成以上步骤，便可为该实例配置一个网络。

#### 3.2.2 域名配置

1. 输入域名名称；
2. 选择协议类型；目前可选两种协议类型，分别是：普通协议与加密协议。
    - 普通协议：选择普通协议的域名时，默认对创建的域名不做处理。
    - 加密协议：通过使用证书（例如SSL证书），来实现对域名的加密。

3. 输入域名地址（选择加密协议时，还需根据输入的域名选择一个对应的证书）；
4. 输入路径、选择端口；
    > 此处的网络默认为在该页面配置的网络。
...
1. 添加项目应用服务或者共享应用服务
    - 项目应用服务：本项目下已有的应用服务；
    - 共享应用服务：组织内其他项目共享出来的应用服务；
2. 为新添加的应用服务重复3.2-3.2步骤
