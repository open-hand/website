...
## 预置应用模板
系统中已经预置了几种应用模板，其中包括 `JavaLib(jar库)`、`MicroServiceFront(前端应用模板)`、`MicroService(微服务应用模板)`在创建应用时可以根据自己的需求选择应用模板，进行快速开发。
...
输入`应用模板编码`，`名称`，`描述`，创建默认最简模板，步骤如下。您也可以通过复制于现有模板，以便节省部分共同操作，提升效率。
   ...
   1. 点击`创建应用模板`按钮；
   1. 输入应用相关信息，进行数据校验
      - 模板编码：编码只能由小写字母、数字、"-"组成，且以小写字母开头，不能以"-"结尾，并且是唯一的，不能与其他的模板相同。
      - 模板名称：名称要求唯一，不能与其他的模板相同。
      - 模板描述：不允许为空。
   1. 点击`创建`按钮；
