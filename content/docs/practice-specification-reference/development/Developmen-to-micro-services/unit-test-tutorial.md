...
```
## 目录结构
![](/docs/practice-specification-reference/development/Developmen-to-micro-services/image/structure.png)
...
```
## 编写测试groovy脚本
添加注解`@SpringBootTest`(webEnvironment = RANDOM_PORT)和@Import(IntegrationTestConfiguration)。继承`Specification`且类名为`Spec`后缀。注入所需要测试的类对象通过`given`(前提条件),`when`(触发条件),`then`(期望结果)。
...
```
## 其他
如果在idea不能直接编译运行测试`groovy`文件，可能是因`idea`没有识别`test/groovy`目录。 

![](/docs/practice-specification-reference/development/Developmen-to-micro-services//image/test-dir.png)
